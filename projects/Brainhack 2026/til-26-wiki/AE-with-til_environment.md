...
**Bombs.** A placed bomb counts down for `timer` steps then detonates in a blast of square radius `2` (so 2 + 1 + 2, where 1 is the center where the bomb is placed). The blast damages agents, bases, and destroys destructible walls in range. Indestructible walls block the blast. The diagram below demonstrates the tiles hit in various cases, with `o` indicating the bomb origin square, `x` indicates tiles hit by the blast, and `-` being indestructible walls.
