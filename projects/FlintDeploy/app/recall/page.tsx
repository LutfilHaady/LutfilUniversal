        ...
        <form onSubmit={handleSearch}>
          <div className="rounded-xl border border-[#2a2a2a] bg-[#111111] p-5 flex flex-col gap-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => { setShowScanner(s => !s); setCameraErrorMsg(null); }}
                title={showScanner ? 'Hide scanner' : 'Scan QR code'}
                className="h-10 w-10 rounded-lg border border-[#2a2a2a] bg-[#161616] text-[#888888] hover:text-[#f5f5f5] flex items-center justify-center transition-colors shrink-0"
              >
            ...
            {showScanner && (
              <div className="rounded-xl overflow-hidden border border-[#2a2a2a]" style={{ height: 180 }}>
                {cameraErrorMsg ? (
                  <div className="h-full flex flex-col items-center justify-center gap-2 bg-[#111] px-5 text-center">
              ...
              <>
                ...
                <div className="rounded-xl border border-[#2a2a2a] bg-[#111111] p-5">
                  ...
                  <div className="flex flex-col gap-2">
                    {([
                      {
                        label: 'Place high-risk sub-batches on Hold',
                        detail: 'Open each high-risk batch from the affected list above, then use the "Change Status → On Hold" button in the batch detail page.',
                      },
                      {
                        label: 'Notify QC team for re-inspection',
                        detail: 'Export this investigation from the Reports page (Compliance tab → Generate CSV) and share with the QC team for follow-up.',
                      },
                      {
                        label: 'Generate recall report for regulatory file',
                        detail: 'Navigate to Reports → Compliance, set the date range to cover the affected production window, then use "Generate PDF" to download the regulatory report.',
                      },
                    ] as { label: string; detail: string }[]).map((item) => (
                      <div
                        key={item.label}
                        className="flex flex-col gap-1.5 px-4 py-3 rounded-lg border border-[#1e1e1e] bg-[#0e0e0e]"
                      >
                        <span className="text-[12.5px] font-medium text-[#f5f5f5]">{item.label}</span>
                        <span className="text-[11.5px] text-[#888888] leading-relaxed">{item.detail}</span>
                      </div>
                    ))}
                  </div>
                </div>
