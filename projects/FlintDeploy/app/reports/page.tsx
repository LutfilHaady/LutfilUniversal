...
import { exportPdf } from '@/lib/reports/pdf-export';
  ...
  return (
    <Shell title="Reports" subtitle="/ Analytics &amp; export">
      <main className="flex-1 min-h-0 px-6 py-5 overflow-y-auto flex flex-col gap-5">
              ...
              onClick={() => {
                try {
                  exportPdf({
                    ...
                    dateFrom,
                    dateTo,
                    batchRows: batchSummaryRows,
                    qcRows,
                    defectRows,
                    compliance,
                  })
                } catch (_err) {
                  // Export failure should not crash the UI
                }
              }}
              className="h-9 px-3 rounded-md border border-[#2a2a2a] bg-[#161616] text-[12px] text-[#888888] hover:text-[#f5f5f5] inline-flex items-center gap-1.5 transition-colors"
            >
        ...
        {/* Tab bar */}
        <div className="flex flex-wrap gap-2 border-b border-[#2a2a2a] pb-5">
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={'px-3.5 py-1.5 rounded-full text-[12px] font-medium border transition-colors ' +
                (tab === t 
                  ...
                  : 'border-[#2a2a2a] bg-[#111111] text-[#888888] hover:text-[#f5f5f5] hover:border-[#3a3a3a]')}>
              {t}
            </button>
          ))}
        </div>
              ...
              <thead>
                <tr className="border-b border-[#2a2a2a]">
                  {['Batch ID', 'Material', 'Quantity', 'Status'].map(h => (
                    <th key={h} className={`px-5 py-3 text-[10.5px] font-mono uppercase tracking-[0.1em] text-[#5a5a5a] ${['Material', 'Quantity'].includes(h) ? 'max-md:hidden' : ''}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1a1a1a]">
                {batchSummaryRows.length === 0 ? (
                  ...
                  <tr key={r.id} className="hover:bg-[#141414] transition-colors">
                    ...
                    <td className="px-5 py-3 text-[12px] text-[#888888] max-md:hidden">{(r.materials as any)?.name ?? '—'}</td>
                    <td className="px-5 py-3 text-[12px] font-mono text-[#888888] max-md:hidden">
              ...
              <thead>
                <tr className="border-b border-[#2a2a2a]">
                  {['QC Type', 'Verdict', 'Checked by', 'Date'].map(h => (
                    <th key={h} className={`px-5 py-3 text-[10.5px] font-mono uppercase tracking-[0.1em] text-[#5a5a5a] ${['Checked by', 'Date'].includes(h) ? 'max-md:hidden' : ''}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1a1a1a]">
                {qcRows.length === 0 ? (
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-[#141414] transition-colors">
                      ...
                      <td className="px-5 py-3 text-[12px] text-[#888888] max-md:hidden">{(r.users as any)?.full_name ?? '—'}</td>
                      <td className="px-5 py-3 text-[12px] font-mono text-[#888888] max-md:hidden">{r.created_at?.slice(0, 16).replace('T', ' ')}</td>
                    </tr>
                  );
                })}
              </tbody>
