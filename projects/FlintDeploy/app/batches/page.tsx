...
export default function BatchesPage() {
  ...
  const filtered = activeCategories.has('All')
    ? batches
    : batches.filter((b) => b.material?.type && activeCategories.has(b.material.type));

  const categoryCounts: Record<string, number> = { All: batches.length };
  for (const cat of MATERIAL_TABS.slice(1)) {
    categoryCounts[cat] = batches.filter((b) => b.material?.type === cat).length;
  }
  ...
  return (
    ...
    >
          ...
          {MATERIAL_TABS.map((cat) => {
            ...
            return (
              <button
                ...
                className={
                  'flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-[12px] font-medium transition-colors whitespace-nowrap border ' +
                  (isActive
              ...
              >
                ...
                {cat}
                <span className="text-[10.5px] font-mono opacity-60 ml-0.5">
      ...
      <div className="shrink-0 px-6 py-2.5 border-t border-[#1a1a1a] bg-[#0a0a0a] flex items-center justify-between">
        <span suppressHydrationWarning className="text-[10.5px] font-mono text-[#5a5a5a]">
