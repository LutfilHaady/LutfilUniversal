...
test.describe('Task 1 — Scan QR Lookup', () => {
  ...
  test('unknown batch number shows Batch Not Found error', async ({ page }) => {
    const FAKE_BATCH = 'MIXC-99999999-A01-01'
    await page.route(`**${SB}/rest/v1/batches*`, async (route) => {
      const url = new URL(route.request().url())
      if (url.searchParams.get('batch_number') === `eq.${FAKE_BATCH}`) {
        return route.fulfill({
          status: 406,
          contentType: 'application/json',
          body: JSON.stringify({
    ...
    await page.goto('/scan')
    await page.getByPlaceholder(/MIXC-20260430-A01-03/i).fill(FAKE_BATCH)
    await page.getByRole('button', { name: 'Go' }).click()

    await expect(page.getByText('Batch Not Found')).toBeVisible()
    await expect(page.getByText(new RegExp(FAKE_BATCH))).toBeVisible()
  })
