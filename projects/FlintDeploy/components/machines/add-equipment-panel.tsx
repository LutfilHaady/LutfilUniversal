...
export function AddEquipmentPanel({ onClose, onAdded, onSaved, initial = null }: AddEquipmentPanelProps) {
  ...
  useEffect(() => {
    if (initial) {
      ...
      setTasks(Array.isArray(initial.checklist_template) ? initial.checklist_template : []);
    }
  }, [initial]);
  ...
  function addTask() {
    ...
    if (!t) return;
    if (tasks.some(x => x.toLowerCase() === t.toLowerCase())) { setTaskInput(''); return; }
    ...
    try {
      if (initial?.id) {
        const { data: updated, error } = await supabase
          ...
          .update(payload)
          .eq('id', initial.id)
          .select('id, equipment_code, name, is_active, supplier_info, process_id, checklist_template, process:processes(name, code)')
          .single();
        if (error) {
          setSubmitError(
            error.code === '23505'
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-[480px] bg-[#0e0e0e] border-l border-[#2a2a2a] flex flex-col shadow-2xl">
        ...
        <form id="aef" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-5">
          ...
          {/* f. Maintenance Checklist Tasks */}
          <div className="flex flex-col gap-2">
            <label className="text-[11px] font-mono uppercase tracking-[0.1em] text-[#888888]">
              Maintenance Checklist Tasks
            </label>
            <span className="text-[10.5px] font-mono text-[#5a5a5a] -mt-1">
              Tasks shown when logging maintenance for this machine.
            </span>
            <div className="flex gap-2">
              <input
                ...
                placeholder="e.g. Rail Lubrication"
                className={inputCls}
              />
              <button
                type="button"
                onClick={addTask}
                aria-label="Add task"
                className="h-9 w-9 shrink-0 rounded-md border border-[#2a2a2a] bg-[#161616] text-[#888888] hover:text-[#f5f5f5] hover:bg-[#1e1e1e] transition-colors inline-flex items-center justify-center"
              >
                <IconPlus size={14} />
              </button>
            </div>
            {tasks.length > 0 && (
              <div className="flex flex-col gap-1.5 mt-1">
                {tasks.map((t, i) => (
                  <div
                    key={`${t}-${i}`}
                    className="flex items-center justify-between gap-3 px-3 py-2 rounded-md border border-[#1e1e1e] bg-[#111]"
                  >
                    <span className="text-[12.5px] text-[#f5f5f5]">{t}</span>
                    <button
                      type="button"
                      onClick={() => removeTask(i)}
                      aria-label={`Remove ${t}`}
                      className="w-6 h-6 rounded flex items-center justify-center text-[#5a5a5a] hover:text-[#f87171] hover:bg-[#1e1e1e] transition-colors shrink-0"
                    >
                      <IconClose size={13} />
                    </button>
                  </div>
