...
import type { Recipe, RecipeProcess, SimplifiedRatioRow } from '@/lib/types';
import { RECIPE_PROCESSES, PROCESS_MATERIALS, PROCESS_PARAM_FIELDS, PROCESS_MATERIAL_TO_DB_NAME, SIMPLIFIED_UNITS } from '@/lib/constants';
...
export function RecipePanel({ mode, recipe, allRecipes, onClose, onSave }: RecipePanelProps) {
  ...
  const [recipeType, setRecipeType] = useState<'standard' | 'simplified' | null>(() => {
    if (mode === 'new') return null;
    return recipe?.params?.simplified_ratios ? 'simplified' : 'standard';
  });
  const [step, setStep] = useState<0 | 1 | 2>(mode === 'new' ? 0 : 1);
  ...
  const [simplifiedRatios, setSimplifiedRatios] = useState<SimplifiedRatioRow[]>(() => {
    return (recipe?.params?.simplified_ratios as SimplifiedRatioRow[]) ?? [];
  });
  ...
  function addSimplifiedRow() {
    setSimplifiedRatios(prev => [...prev, { material: '', amount: 1, unit: 'kg' }]);
  }
  function updateSimplifiedRow(index: number, field: keyof SimplifiedRatioRow, value: string | number) {
    setSimplifiedRatios(prev => prev.map((r, i) => i === index ? { ...r, [field]: value as never } : r));
  }
  function removeSimplifiedRow(index: number) {
    setSimplifiedRatios(prev => prev.filter((_, i) => i !== index));
  }

  const fields = PROCESS_PARAM_FIELDS[process];

  const step1Valid = name.trim() !== '';
  const step2Valid = recipeType === 'simplified'
    ? simplifiedRatios.length > 0 && simplifiedRatios.every(r => r.material.trim() !== '' && r.amount > 0)
    : isDraftValid(fields, draft);
  ...
  async function handleSave() {
    ...
    const builtParams = recipeType === 'simplified'
      ? { simplified_ratios: simplifiedRatios }
      : buildParams(fields, draft);
        ...
        <div className="flex items-start justify-between px-6 py-5 border-b border-[#2a2a2a]">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-center gap-2 text-[11px] font-mono">
              {recipeType === null ? (
                <span className="text-[#22c55e]">0. Select Type</span>
              ) : (
                <>
                  <span className={step === 1 ? 'text-[#22c55e]' : 'text-[#5a5a5a]'}>1. Details</span>
                  <span className="text-[#3a3a3a]">→</span>
                  <span className={step === 2 ? 'text-[#22c55e]' : 'text-[#5a5a5a]'}>
                    {recipeType === 'simplified' ? '2. Material Ratios' : '2. Parameters'}
                  </span>
                </>
              )}
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4">
          ...
          {step === 0 && (
            <div className="flex flex-col gap-3">
              <div className="text-[13px] text-[#f5f5f5] mb-2">What kind of recipe would you like to create?</div>
              <button
                type="button"
                onClick={() => { setRecipeType('simplified'); setStep(1); }}
                className="flex flex-col items-start gap-1 p-4 rounded-xl border border-[#2a2a2a] bg-[#161616] hover:border-[#22c55e] transition-colors text-left"
              >
                <div className="text-[14px] font-semibold text-[#22c55e]">Simplified Recipe</div>
                <div className="text-[12px] text-[#888888]">Only define material mix ratios. No complex machine parameters.</div>
              </button>
              <button
                type="button"
                onClick={() => { setRecipeType('standard'); setStep(1); }}
                className="flex flex-col items-start gap-1 p-4 rounded-xl border border-[#2a2a2a] bg-[#161616] hover:border-[#3b82f6] transition-colors text-left"
              >
                <div className="text-[14px] font-semibold text-[#3b82f6]">Standard Recipe</div>
                <div className="text-[12px] text-[#888888]">Full configuration including all machine parameters and variables.</div>
              </button>
            </div>
          )}
          ...
          {step === 2 && (
            <>
              ...
              {recipeType === 'simplified' ? (
                <div className="flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                    <div className={labelCls}>Material Ratios</div>
                    <button
                      type="button"
                  ...
                  {simplifiedRatios.length === 0 ? (
                    <div className="text-[12px] text-[#5a5a5a] italic">No materials added yet.</div>
                  ) : (
                      ...
                      <table className="w-full text-left">
                        <thead>
                          <tr className="bg-[#111]">
                            ...
                            <th className="px-3 py-2 text-[10px] font-mono text-[#5a5a5a]">Amount</th>
                            <th className="px-3 py-2 text-[10px] font-mono text-[#5a5a5a]">Unit</th>
                            <th className="px-3 py-2 w-8"></th>
                          </tr>
                        </thead>
                        <tbody>
                          {simplifiedRatios.map((r, ri) => (
                            <tr key={ri} className="border-t border-[#1a1a1a]">
                              <td className="px-3 py-2">
                                <input
                                  type="text"
                                  value={r.material}
                                  onChange={e => updateSimplifiedRow(ri, 'material', e.target.value)}
                                  placeholder="e.g. Water"
                                  className="h-8 px-2 w-full rounded-md border border-[#2a2a2a] bg-[#161616] text-[12px] text-[#f5f5f5] outline-none focus:border-[#22c55e]"
                                />
                              </td>
                              <td className="px-3 py-2 w-24">
                                <input
                                  ...
                                  step="any"
                                  value={r.amount}
                                  onChange={e => updateSimplifiedRow(ri, 'amount', parseFloat(e.target.value) || 0)}
                                  className="h-8 px-2 w-full rounded-md border border-[#2a2a2a] bg-[#161616] text-[12px] text-[#f5f5f5] outline-none focus:border-[#22c55e]"
                                />
                              </td>
                              <td className="px-3 py-2 w-20">
                                <select
                                  value={r.unit}
                                  onChange={e => updateSimplifiedRow(ri, 'unit', e.target.value)}
                                  className="h-8 px-2 w-full rounded-md border border-[#2a2a2a] bg-[#161616] text-[12px] text-[#f5f5f5] outline-none focus:border-[#22c55e] cursor-pointer"
                                >
                                  {SIMPLIFIED_UNITS.map(u => <option key={u} value={u}>{u}</option>)}
                                </select>
                              </td>
                              <td className="px-3 py-2 text-center">
                                <button
                                  type="button"
                                  onClick={() => removeSimplifiedRow(ri)}
                                  className="text-[#888888] hover:text-[#ef4444] transition-colors inline-flex mt-1"
                                >
                                  <IconClose size={14} />
                                </button>
                              </td>
        ...
        <div className="px-6 py-4 border-t border-[#2a2a2a] flex gap-3">
          {step === 0 ? (
            <button
              type="button"
              onClick={onClose}
              className="w-full h-9 rounded-md border border-[#2a2a2a] text-[13px] text-[#888888] hover:text-[#f5f5f5] hover:bg-[#1e1e1e] transition-colors"
            >
              Cancel
            </button>
          ) : step === 1 ? (
            <>
              <button
                type="button"
                onClick={() => mode === 'new' ? setStep(0) : onClose()}
