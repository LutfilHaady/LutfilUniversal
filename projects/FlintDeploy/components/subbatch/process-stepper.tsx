        ...
        {steps.map((step, i) => (
          <div key={step.process_id} className="flex items-center flex-1 min-w-0">
            <div className="flex flex-col items-center gap-1.5 min-w-0">
              ...
              <div className="text-center min-w-0 w-full overflow-hidden px-0.5">
                <div className="text-[11.5px] font-medium leading-tight text-[#5a5a5a] break-words">{step.name}</div>
            ...
            {i < steps.length - 1 && (
              <div className="flex-1 min-w-2 h-0.5 mx-2 mt-[-20px]" style={{ background: '#1e1e1e' }} />
            )}
          </div>
        ))}
      </div>
