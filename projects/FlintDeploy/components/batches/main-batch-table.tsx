      ...
      <table className="w-full text-left">
            ...
            {['Batch ID', 'Material', 'Supplier', 'Total Qty', 'Remaining', 'Sub-Batches', 'Lot Reference', 'Created', ''].map((h) => (
              <th
                ...
                className={`px-5 py-3 text-[10.5px] font-mono uppercase tracking-[0.1em] text-[#5a5a5a] whitespace-nowrap ${
                  ['Supplier', 'Remaining', 'Lot Reference', 'Created'].includes(h) ? 'max-md:hidden' : ''
                }`}
              >
                {h}
              </th>
        ...
        <tbody className="divide-y divide-[#1a1a1a]">
          {batches.map((batch) => {
            ...
            const materialType = batch.material?.type ?? '—'
            const accentColor = CATEGORY_TONES[materialType] ?? '#888888'
                  ...
                  <td className="px-5 py-3">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[12px] font-medium text-[#f5f5f5]">{materialType}</span>
                      </div>
                      <span className="text-[11px] text-[#888888] pl-4">{materialName}</span>
                    </div>
                  </td>
                  <td className="px-5 py-3 text-[12px] text-[#888888] max-md:hidden">{batch.intake?.supplier_name ?? '—'}</td>
                  <td className="px-5 py-3 text-[12px] font-mono text-[#888888]">{totalQty}</td>
                  <td className="px-5 py-3 max-md:hidden">
                    <div className="flex items-center gap-2">
                        ...
                        <thead>
                          <tr className="border-b border-[#1a1a1a]">
                            {['Sub-Batch ID', 'Quantity', 'Location', 'Status', 'Operator'].map((h) => (
                              <th 
                                key={h} 
                                className={`${h === 'Sub-Batch ID' ? 'pl-10 pr-4' : 'px-4'} py-2 text-[10px] font-mono uppercase tracking-[0.1em] text-[#3a3a3a] whitespace-nowrap ${['Location', 'Operator'].includes(h) ? 'max-md:hidden' : ''}`}
                              >
                                {h}
                              </th>
                            ))}
