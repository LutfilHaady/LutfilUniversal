...
export interface SimplifiedRatioRow {
  material: string;
  amount: number;
  unit: string;
}
...
export interface Recipe {
  ...
  is_active: boolean
  created_at: string
  params: Record<string, unknown> // For simplified recipes, contains { simplified_ratios: SimplifiedRatioRow[] }
  notes: string | null
...
export interface Unit {
  ...
  sub_batch_id: string
  created_at: string
  // Joined source sub-batch (for showing its batch_number instead of the UUID)
  sub_batch?: { batch_number: string } | null
}
...
export interface Lot {
  ...
  created_at: string
  // From joins
  lot_sub_batches: { sub_batch_id: string; sub_batch?: { batch_number: string; parent_batch_id: string | null } | null }[]
  units?: Unit[]
}
