...
async function resolveUser(session: Session): Promise<AuthUser> {
  ...
  const avatarUrl = (session.user.user_metadata?.avatar_url as string | undefined) || undefined

  return { id: session.user.id, email: session.user.email ?? '', name, role, avatarUrl }
}
