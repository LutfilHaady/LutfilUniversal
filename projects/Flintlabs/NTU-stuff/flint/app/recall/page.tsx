...
import { SUB_BATCHES, MAIN_BATCHES } from '@/lib/data';

type InvestigationState = 'idle' | 'searching' | 'results';

const AFFECTED_MOCK = [
  { id: 'MIXC-20260430-A01-03', type: 'Sub-Batch',     status: 'InProgress', risk: 'high',   reason: 'Failed coating QC' },
  { id: 'MIXC-20260430-A01',    type: 'Batch',          status: 'InProgress', risk: 'medium', reason: 'Parent batch under review' },
  { id: 'FOIL-20260428-AL-02',  type: 'Input Material', status: 'OnHold',     risk: 'medium', reason: 'Shared substrate foil lot' },
  { id: 'MIXC-20260430-A01-01', type: 'Sub-Batch',     status: 'Released',   risk: 'low',    reason: 'Same parent, earlier run' },
  { id: 'MIXC-20260430-A01-02', type: 'Sub-Batch',     status: 'Released',   risk: 'low',    reason: 'Same parent, earlier run' },
];
        ...
        <form onSubmit={handleSearch}>
          <div className="rounded-xl border border-[#2a2a2a] bg-[#111111] p-5 flex flex-col gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-[12px] text-[#888888] mt-0.5">
                  Enter a batch ID, sub-batch ID, material lot, or supplier code to trace all affected records.
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  placeholder="MIXC-20260430-A01-03, FOIL-20260428-AL-02, Targray…"
            ...
            <div className="flex flex-wrap gap-2">
              <span className="text-[11px] text-[#5a5a5a]">Recent:</span>
              {['MIXC-20260430-A01-03', 'FOIL-20260428-AL-02', 'Targray Industries'].map((s) => (
                <button
                  key={s}
                  type="button"
                  onClick={() => setQuery(s)}
                  className="text-[11px] font-mono text-[#888888] hover:text-[#f5f5f5] transition-colors px-2 py-0.5 rounded border border-[#2a2a2a] hover:border-[#3a3a3a]"
                >
                  {s}
                </button>
              ))}
        ...
        {state === 'results' && (
          <>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: 'Affected Records', value: AFFECTED_MOCK.length.toString(), color: '#ef4444' },
                { label: 'High Risk', value: AFFECTED_MOCK.filter((a) => a.risk === 'high').length.toString(), color: '#ef4444' },
                { label: 'Batches Impacted', value: '1', color: '#f59e0b' },
              ].map((k) => (
            ...
            >
              ...
              <div>
                ...
                <div className="text-[12px] text-[#888888] mt-1">
                  {AFFECTED_MOCK.length} records identified across{' '}
                  {new Set(AFFECTED_MOCK.map((a) => a.type)).size} record types.
                  High-risk items require immediate review.
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-[#2a2a2a] bg-[#111111] overflow-hidden">
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-[#1a1a1a]">
                  {AFFECTED_MOCK.map((item) => {
                    const risk = RISK_STYLES[item.risk as keyof typeof RISK_STYLES];
                    return (
                      <tr
                        key={item.id}
            ...
            <div className="rounded-xl border border-[#2a2a2a] bg-[#111111] p-5">
              ...
              <div className="flex flex-col gap-2">
                {[
                  { label: 'Place high-risk sub-batches on Hold', status: 'pending' },
                  { label: 'Notify QC team for re-inspection', status: 'pending' },
                  { label: 'Flag supplier lot FOIL-20260428-AL-02', status: 'pending' },
                  { label: 'Generate recall report for regulatory file', status: 'pending' },
                ].map((action) => (
                  <div
                    key={action.label}
                    className="flex items-center justify-between px-4 py-3 rounded-lg border border-[#1e1e1e] bg-[#0e0e0e]"
                  >
                    <span className="text-[12.5px] text-[#888888]">{action.label}</span>
                    <button className="text-[11.5px] font-medium text-[#93c5fd] hover:text-white transition-colors">
                      Execute →
                    </button>
                  </div>
                ))}
        ...
        {state === 'idle' && (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <div className="text-[13px] text-[#5a5a5a] text-center max-w-xs">
              Enter any batch ID, sub-batch ID, material lot, or supplier name to start a traceability investigation.
            </div>
          </div>
        )}
