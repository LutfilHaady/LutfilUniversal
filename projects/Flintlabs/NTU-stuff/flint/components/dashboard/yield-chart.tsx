...
import { YIELD_TREND } from '@/lib/data';
...
function buildPath(points: { day: string; yield: number }[]) {
  ...
  const area =
    `${line} L ${toX(points.length - 1).toFixed(1)} ${(PAD_T + chartH).toFixed(1)} L ${PAD_L.toFixed(1)} ${(PAD_T + chartH).toFixed(1)} Z`;
...
export function YieldChart() {
  ...
  const points = YIELD_TREND;
  const { line, area, toX, toY } = buildPath(points);
  const current = points[points.length - 1].yield;
    ...
    <div className="rounded-xl border border-[#2a2a2a] bg-[#111111] flex flex-col">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-[#2a2a2a]">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] text-[#22c55e] bg-[rgba(34,197,94,0.1)] border border-[rgba(34,197,94,0.25)] px-2 py-0.5 rounded font-mono">
            +0.8%
          </span>
        </div>
      ...
      <div className="flex-1 px-2 py-2 relative">
        ...
        >
          ...
          {/* Y-axis gridlines */}
          {[90, 92, 94, 96, 98].map((v) => {
            const { toY } = buildPath(points);
            const y = toY(v);
            return (
              <g key={v}>
