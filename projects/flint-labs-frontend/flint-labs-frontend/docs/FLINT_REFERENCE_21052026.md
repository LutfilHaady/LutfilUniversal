...
| `qc_check_definitions` | `acceptance_criteria` | `acceptance_criteria_text` (also has `acceptance_criteria_min` numeric, `acceptance_criteria_max` numeric) |
...
**Gap:** Current QC wizard is generic pass/fail. Needs per-item QC inputs and the calibration start-up flow.
...
| **Dismiss** | ✅ Engineer/Admin dismiss button sets `resolved_at = now()` then `mutate('alerts')`; active views filter `resolved_at IS NULL`. |
| **Auto-resolve** | ✅ (2026-06-04) `scanAlerts()` re-evaluates open scan-generated alerts each pass: any whose `dedup_key` is no longer in the freshly-computed desired set is auto-resolved (`resolved_at = now()`), scoped to **enabled** rule keys so disabling a rule doesn't sweep-resolve its alerts. Idempotent — `.is('resolved_at', null)` on the update, no flip-flop. **Caveat:** `alerts` has no column distinguishing auto-resolve from manual dismiss — both set `resolved_at` (`// TODO` in code). |
...
| `get_process_route` | Returns ordered process steps for a given material, using `materials.first_process_id` + `processes.sequence_hint`. Use to drive the dynamic process stepper. |
...
| **3B** Alert triggers | DB triggers → insert into `alerts` on QC fail / quarantine / maintenance overdue | 1 hr |
...
| `/log/process-step` | Process step logging wizard | ✅ Wired (2026-06-04) — `process_runs` INSERT with `equipment_id`/`recipe_id`; calibration gate; PATCH stamps `end_date`/`end_time`/`AwaitingQC` on submit |
| `/log/mixing/[batchId]` | Operator mixing page — live timer card, step history, Add Step bottom sheet | ✅ Wired to Supabase (2026-05-21) |
| `/log/qc` | QC logging wizard | ✅ Wired (2026-06-02) — inserts qc_check_results, creates output batch on pass, holds input batch on fail |
...
| Batch list & detail | ✅ | ❌ | Process stepper needs per-material dynamic routes — use `get_process_route` RPC |
...
| Lot list & detail | ✅ | ❌ | |
| Process logging wizard | ✅ | ✅ | Wired 2026-06-01; equipment_id/recipe_id + calibration gate + AwaitingQC closure wired 2026-06-04 |
...
| Machines / Maintenance | ✅ | ✅ | Wired 2026-06-02 — Add/Edit Equipment write to `equipment` (process dropdown uses real UUIDs, 23505 dup-code handling); Log Maintenance writes to `equipment_maintenance` (`performed_by`=user.id; reviewed_by/approved_by null pending user-picker) |
...
| PDF export | ⚠️ | ❌ | Toast + `// TODO` — not implemented |
...
| `staff_code` not auto-generated on Add User — shows UUID as fallback | Medium — add field to Add User modal or auto-generate from role + count |
...
| ~~Sub-batch creation~~ | ✅ Done 2026-06-03 — `components/batches/create-subbatch-drawer.tsx` inserts a child `batches` row (`parent_batch_id` set, `status='InProgress'`, inherited `material_id`, generated `{process.code}-YYYYMMDD-Ann` number). Open to all roles. **Split integrity completed 2026-06-05:** parent `current_quantity` now deducted (guarded read→write; over-allocation blocked before any write), initial `batch_status_changes` audit row written for the new sub-batch, and machine/recipe/operator persisted as an `InProgress` `process_runs` row (`output_batch_id`=sub-batch). NB: non-atomic (no client transaction / no deduct RPC) — `// TODO` left in code; candidate for a backend RPC. |
...
| Material intake form (§5.1) | ✅ Done 2026-06-04 — `batch_raw_material_intake` INSERT wired in `create-batch-modal`; initial audit row + QR success screen added. Note: no `notes` column in the table so form notes field is not persisted. |
...
| Dynamic process routes | High — use `get_process_route` RPC (now live) |
| Machine add/edit/deactivate | Medium |
...
| QR camera | Placeholder only |
| Search | Header input not functional |
...
| Process route (all materials show same steps) | Each material has a unique route (see §2) — use `get_process_route` RPC | ❌ Not done |
