...
export function CreateSubBatchDrawer({
  ...
  // Processes + operators load once on mount.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      ...
      if (cancelled) return;
      setProcesses(((procData ?? []) as DbProcess[]).map((p) => ({ id: p.id, code: p.code, name: p.name })));
      setOperators(
  ...
  const previewId = selectedProcess ? `${selectedProcess.code}-${todayCompact()}-A01` : null;
  ...
  // The real sequence is computed at submit time so concurrent sub-batches don't collide.
  async function nextBatchNumber(code: string): Promise<string> {
    const prefix = `${code}-${todayCompact()}-`;
    const { data } = await supabase
      .from('batches')
      .select('batch_number')
      .like('batch_number', `${prefix}%`);
    const seq = (data?.length ?? 0) + 1;
    return `${prefix}A${String(seq).padStart(2, '0')}`;
  }
            ...
            <div className="rounded-md border border-[#363636] px-4 py-3.5" style={{ background: '#1a1a1a' }}>
              ...
              {previewId ? (
                ...
                <div className="font-mono text-[20px] font-semibold text-[#5a5a5a] tabular-nums tracking-tight">
                  <span className="text-[#3a3a3a]">····</span>-{todayCompact()}-A01
                </div>
              )}
