                    ...
                    <Fragment key={r.id}>
                      ...
                      >
                        ...
                        <td className="px-5 py-3">
                          <div className={`text-[12.5px] font-medium ${muted ? 'text-[#4a4a4a]' : 'text-[#f5f5f5]'}`}>{r.name}</div>
                          <div className={`text-[10.5px] font-mono ${muted ? 'text-[#3a3a3a]' : 'text-[#5a5a5a]'}`}>{r.recipe_number ?? r.id}</div>
                        </td>
                        <td className={`px-5 py-3 text-[12px] ${muted ? 'text-[#3a3a3a]' : 'text-[#888888]'}`}>{r.process?.name ?? '—'}</td>
                        <td className={`px-5 py-3 font-mono text-[12px] ${muted ? 'text-[#3a3a3a]' : 'text-[#888888]'}`}>v{r.version}</td>
                        <td className={`px-5 py-3 text-[12px] ${muted ? 'text-[#3a3a3a]' : 'text-[#888888]'}`}>{r.creator?.full_name ?? '—'}</td>
                        <td className={`px-5 py-3 text-[12px] font-mono ${muted ? 'text-[#3a3a3a]' : 'text-[#888888]'}`}>{r.created_at}</td>
                                ...
                                {canEdit && (
                                  <>
                                    <button
                                      ...
                                      className="h-7 px-3 rounded border border-[#2a2a2a] text-[11.5px] text-[#888888] hover:text-[#f5f5f5] hover:border-[#3a3a3a] hover:bg-[#1a1a1a] transition-colors"
                                    >
                                      Edit Parameters
                                    </button>
                                    <button
                                ...
                                <div className="flex flex-col gap-4">
                                  {scalars.length > 0 && (
                                    <div className="grid grid-cols-4 gap-2">
                                      {scalars.map(f => (
                                        <div key={f.key} className="rounded-lg border border-[#1e1e1e] bg-[#111] px-3 py-2">
                                    ...
                                    <div key={f.key}>
                                      ...
                                      <div className="grid grid-cols-6 gap-2">
                                        {Array.from({ length: f.length }).map((_, i) => (
                                          <div key={i} className="rounded-lg border border-[#1e1e1e] bg-[#111] px-3 py-2">
                                        ...
                                        <table className="w-full text-left">
                                          <thead>
                                            <tr className="bg-[#0f0f0f]">
                                              {f.columns.map(c => (
                                                <th key={c.key} className="px-3 py-2 text-[10px] font-mono text-[#5a5a5a] whitespace-nowrap">
                                          ...
                                          <tbody>
                                            {((params[f.key] as Record<string, unknown>[] | undefined) ?? []).map((row, ri) => (
                                              <tr key={ri} className="border-t border-[#1a1a1a]">
                                                {f.columns.map(c => (
                                                  <td key={c.key} className="px-3 py-2 text-[12px] font-mono text-[#f5f5f5] whitespace-nowrap">{fmtParam(row?.[c.key])}</td>
                                                ))}
                                              </tr>
                                            ))}
                                          </tbody>
