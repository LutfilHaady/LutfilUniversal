...
import { IconChevronRight, IconPrint } from '@/components/icons'
...
export default function LotDetailPage() {
  ...
  const breadcrumb = (
    <div className="flex items-center gap-2 min-w-0">
      ...
      <span className="font-mono text-[12.5px] text-[#f5f5f5] truncate">{lot?.lot_number ?? params.id}</span>
    </div>
  )
              ...
              <div className="grid gap-5" style={{ gridTemplateColumns: '1fr minmax(0,320px)' }}>
                    ...
                    <tbody className="divide-y divide-[#1a1a1a]">
                      {(lot.units ?? []).map(u => (
                        <tr key={u.serial} className="hover:bg-[#141414] transition-colors">
                          ...
                          <td className="px-5 py-2.5 font-mono text-[12px] text-[#93c5fd]">{u.sub_batch_id}</td>
                ...
                <div className="rounded-xl border border-[#2a2a2a] bg-[#111111] overflow-hidden self-start">
                  ...
                  <div className="flex flex-col divide-y divide-[#1a1a1a]">
                    {lot.lot_sub_batches.map(({ sub_batch_id }) => (
                      <Link key={sub_batch_id} href={`/batches/${sub_batch_id}`}
                        className="flex items-center justify-between px-5 py-3 hover:bg-[#141414] transition-colors">
                        <span className="font-mono text-[12px] text-[#93c5fd] hover:text-white transition-colors">{sub_batch_id}</span>
