...
export default function MachinesPage() {
  ...
  function handleMaintenanceLogged(code: string, entry: any, nextDue?: string) {
    const machineName = logPanelFor?.name ?? code;
    // Normalize to the Supabase MaintenanceEntry shape
    const normalized = {
      id: `local-${Date.now()}`,
      equipment_id: code,
      maintenance_date: entry.date ?? entry.maintenance_date ?? new Date().toISOString().slice(0,10),
      next_due_date: nextDue ?? entry.nextDue ?? entry.next_due_date ?? null,
    ...
    setMachines(prev => prev.map(m => {
      ...
      return { ...m, equipment_maintenance: [normalized, ...m.equipment_maintenance] };
    }));
    setLogPanelFor(null);
