...
import { IconChevronRight, IconPlus, IconExternal, IconLock, IconBatches } from '@/components/icons';
...
export default function MainBatchDetailPage() {
  ...
  // Remaining = parent current_quantity − sum of sub-batch current_quantity (per spec).
  const allocated = subBatches.reduce((sum, s) => sum + (s.qty ?? 0), 0);
  const remaining = (batch?.currentQty ?? 0) - allocated;
  const remainingRounded = Math.round(remaining * 100) / 100;
  const allocatedRounded = Math.round(allocated * 100) / 100;
  const remainingPct =
    batch?.currentQty && batch.currentQty > 0
      ? `${Math.round((remaining / batch.currentQty) * 100)}% unused`
      : null;
