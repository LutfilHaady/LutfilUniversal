...
export default function SubBatchDetailPage() {
  ...
  const isMixingBatch = batch ? (batch.batch_number.startsWith('MIXC') || batch.batch_number.startsWith('MIXE')) : false
  ...
  const breadcrumb = (
    <div className="flex items-center gap-2 min-w-0">
      ...
      <IconChevronRight size={13} className="text-[#5a5a5a] shrink-0" />
      <span className="font-mono text-[12.5px] text-[#f5f5f5] truncate">{batch?.batch_number ?? params.subId}</span>
    </div>
  )
              ...
              <div className="grid gap-5" style={{ gridTemplateColumns: 'minmax(0,3fr) minmax(0,2fr)' }}>
                {isMixingBatch ? (
                  <MixingStepsPanel
                    ...
                    canAddSteps={!stepsLoading && batch.status === 'InProgress'}
                  />
                ) : (
