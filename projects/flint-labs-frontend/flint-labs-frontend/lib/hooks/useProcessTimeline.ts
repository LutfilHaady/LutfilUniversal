...
async function fetchProcessRuns(
  ...
  return unique.map((run: any) => {
    const started_at = run.start_date ? `${run.start_date}T${run.start_time ?? '00:00:00'}Z` : new Date().toISOString()
    const completed_at = run.end_date ? `${run.end_date}T${run.end_time ?? '00:00:00'}Z` : null
